  ...
  <div class="container mx-auto w-full flex flex-col md:flex-row pt-2">
    <div class="flex md:w-1/4 flex-col md:items-end p-1">
      ...
      <div class="w-full">
        <div tabindex="0" class="collapse collapse-plus border border-base-300 bg-base-100 rounded-box">
          ...
          <div class="collapse-content">
            ...
            <!-- results -->
            <div>
              <span v-for="char of idsResults" class="cursor-pointer hover:underline hover:bg-primary hover:text-white"
                @click="updateHanzi(char)">{{ char }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex w-full justify-between gap-2 pt-2">
        <input type="text" placeholder="ここに入力" class="input input-bordered input-primary w-2/3 max-w-xs"
          v-model="hanzi" />
        <button class="btn btn-primary w-1/3" @click="move(hanzi)">検索</button>
      </div>
      ...
      <div class="pt-2 w-full">
        <div tabindex="0" class="collapse collapse-plus border border-base-300 bg-base-100 rounded-box">
          ...
          <div class="collapse-content">
            <!-- list  -->
            <span class="cursor-pointer hover:bg-primary hover:text-white" v-for="item of variantList"
              @click="move(item)">{{ item }}</span>
